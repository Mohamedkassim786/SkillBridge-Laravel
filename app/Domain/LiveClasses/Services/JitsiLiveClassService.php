...
namespace App\Domain\LiveClasses\Services;
...
class JitsiLiveClassService
{
    ...
    {
        $prefix = config('jitsi.room_prefix', 'live_class_');
        return $prefix . strtolower((string) Str::ulid()) . '_' . Str::random(8);
    }
    ...
    public function canJoin(LiveClass $liveClass, User $user): bool
    {
        ...
        // Student authorization check: Must be enrolled in the course (active or completed) and assigned to batch if set
    ...
    {
        $domain = config('jitsi.domain', config('services.jitsi.domain', 'meet.jit.si'));
        $useJwt = config('jitsi.use_jwt', config('services.jitsi.use_jwt', false));
    ...
    {
        $appId = config('jitsi.app_id', config('services.jitsi.app_id'));
        $secret = config('jitsi.app_secret', config('services.jitsi.app_secret'));

        if (empty($appId) || empty($secret)) {
            return '';
        }

        $domain = config('jitsi.domain', config('services.jitsi.domain'));
        $ttl = config('jitsi.token_ttl', 3600);
