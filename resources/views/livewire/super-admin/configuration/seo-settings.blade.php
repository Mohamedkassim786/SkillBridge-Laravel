    ...
    <div style="background-color: #251237; border: 1px solid rgba(241,81,83,0.3);" class="rounded-3xl p-6 shadow-xl space-y-6 text-white">
        <form wire:submit.prevent="saveSeo" class="space-y-6">
            <div>
                ...
                <input type="text" wire:model="globalMetaTitle" required style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-bold">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-300 mb-1">Global Meta Description Tag</label>
                <textarea wire:model="globalMetaDescription" rows="3" required style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full p-4 rounded-xl text-xs focus:outline-none"></textarea>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">OpenGraph Image URL</label>
                    <input type="text" wire:model="ogImageUrl" required style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">Canonical URL Origin</label>
                    <input type="text" wire:model="canonicalUrl" required style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-300 mb-1">Robots.txt Content Editor</label>
                <textarea wire:model="robotsTxt" rows="5" required style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full p-4 rounded-xl text-xs font-mono focus:outline-none"></textarea>
            </div>
