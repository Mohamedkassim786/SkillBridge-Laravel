<div class="space-y-6 max-w-4xl mx-auto">
        ...
        <form wire:submit.prevent="saveMailConfig" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" wire:model="smtpHost" required style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">SMTP Port</label>
                    <input type="number" wire:model="smtpPort" required style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">SMTP Username</label>
                    <input type="text" wire:model="smtpUsername" style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">SMTP Password (Masked)</label>
                    <input type="password" wire:model="smtpPassword" placeholder="••••••••••••" style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>

                <div class="sm:col-span-2">
                    <label class="block text-xs font-bold text-slate-300 mb-1">Mail Sender Address</label>
                    <input type="email" wire:model="mailFromAddress" required style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>
            </div>
    ...
    <div style="background-color: #251237; border: 1px solid rgba(241,81,83,0.3);" class="rounded-3xl p-6 shadow-xl space-y-4 text-white">
        ...
        <form wire:submit.prevent="sendTestMessage" class="flex flex-col sm:flex-row gap-3">
            <input type="email" wire:model="testEmailRecipient" required placeholder="Enter recipient email address..." style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="flex-1 px-4 py-2.5 rounded-xl text-xs font-mono">
