        ...
        <form wire:submit.prevent="saveSecurity" class="space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <select wire:model="enforce2FA" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-bold">
                ...
                <div>
                    ...
                    <input type="number" wire:model="sessionTimeoutMins" required min="15" max="1440" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">Max Login Failed Attempt Limit</label>
                    <input type="number" wire:model="maxLoginAttempts" required min="3" max="10" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-300 mb-1">IP Address Blocklist (Comma-separated IPs)</label>
                <textarea wire:model="ipBlocklist" rows="3" placeholder="192.168.1.100, 10.0.0.45" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full p-4 rounded-xl text-xs font-mono focus:outline-none"></textarea>
            </div>
