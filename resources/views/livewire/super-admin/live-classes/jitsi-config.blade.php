        ...
        <form wire:submit.prevent="saveSettings" class="space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" wire:model="domain" required placeholder="meet.jit.si or jitsi.yourdomain.com" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">JWT Authentication Mode</label>
                    <select wire:model="useJwt" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-bold">
                ...
                <div>
                    ...
                    <input type="text" wire:model="appId" placeholder="vpaas-magic-cookie-..." style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">JWT Secret / Private Key (Masked)</label>
                    <input type="password" wire:model="appSecret" placeholder="••••••••••••••••" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">Room Naming Pattern</label>
                    <input type="text" wire:model="roomNamingPattern" placeholder="live_class_{ulid}" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">JWT Token Expiration (Minutes)</label>
                    <input type="number" wire:model="tokenExpirationMins" min="15" max="1440" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-4 py-2.5 rounded-xl text-xs font-mono">
                </div>
            </div>
