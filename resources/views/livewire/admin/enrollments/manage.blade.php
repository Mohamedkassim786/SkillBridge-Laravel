    ...
    <div class="p-6 rounded-3xl bg-gradient-to-r from-[#0B1F3A] to-slate-900 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <div class="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-widest">
                ...
                <span>•</span>
                <span class="text-[#D62828]">Student Enrollments & Access Manager</span>
            </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search student or course..." class="px-4 py-2.5 rounded-xl bg-slate-800 text-white text-xs font-bold border border-slate-700 focus:ring-2 focus:ring-[#D62828] outline-none">
            ...
            <tbody class="divide-y divide-slate-100 font-medium text-slate-800">
                @forelse ($enrollments as $enr)
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        <td class="py-3 px-3">
                            <div class="font-bold text-[#0B1F3A]">{{ $enr->user?->name ?? 'User' }}</div>
                        ...
                        <td class="py-3 px-3">
                            <div class="flex items-center gap-2">
                                <div class="w-24 bg-slate-200 h-2 rounded-full overflow-hidden">
                                    <div class="bg-[#D62828] h-full" style="width: {{ $enr->progress_percent }}%"></div>
                                </div>
    ...
    <div x-show="modalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="w-full max-w-lg rounded-3xl bg-white p-6 sm:p-8 shadow-2xl space-y-6" @click.away="modalOpen = false">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                <h3 class="text-lg font-extrabold text-[#0B1F3A]">Enroll Student into Course</h3>
            ...
            <form wire:submit.prevent="enrollStudent" class="space-y-4">
                <div>
                    ...
                    <select wire:model="user_id" class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-[#D62828]">
                ...
                <div>
                    ...
                    <select wire:model="course_id" class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-[#D62828]">
