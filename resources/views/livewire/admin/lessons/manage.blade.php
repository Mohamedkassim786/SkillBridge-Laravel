<div class="space-y-8" x-data="{ modalOpen: @entangle('showModal'), moduleModalOpen: @entangle('showModuleModal') }">
    ...
    <div class="p-6 rounded-3xl bg-gradient-to-r from-[#251237] to-[#1e0d2d] text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 border border-purple-800/40">
        <div>
            <div class="flex items-center gap-2 text-xs font-semibold text-slate-400 uppercase tracking-widest">
                ...
                <span>•</span>
                <span class="text-[#f15153]">Video & Curriculum Manager</span>
            </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <select wire:model.live="selectedCourseId" class="px-4 py-2.5 rounded-xl bg-slate-800 text-white text-xs font-bold border border-slate-700 focus:ring-2 focus:ring-[#f15153] outline-none">
        ...
        <div class="space-y-6">
            @forelse ($modules as $module)
                <div class="p-6 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <h2 class="text-base font-extrabold text-[#251237] flex items-center gap-2">
                            <span class="w-2.5 h-2.5 rounded-full bg-[#f15153]"></span>
                        ...
                        @forelse ($module->lessons->sortBy('sort_order') as $lesson)
                            <div class="p-4 rounded-2xl bg-slate-50 hover:bg-slate-100 border border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all">
                                <div class="flex items-start sm:items-center gap-3">
                                    <div class="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-lg font-bold text-[#251237] shadow-xs">
                                        🎬
                                    </div>
                                    <div>
            ...
            @empty
                <div class="p-8 rounded-3xl bg-white border border-slate-200 text-center text-slate-500 space-y-4">
                    ...
                    <button wire:click="openModuleModal" class="px-4 py-2 rounded-xl bg-[#251237] hover:bg-slate-800 text-white text-xs font-bold shadow-md">
                        + Create First Module
                    </button>
                </div>
            @endforelse
    ...
    <div x-show="moduleModalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl space-y-6" @click.away="moduleModalOpen = false">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                <h3 class="text-base font-extrabold text-[#251237]">Create New Course Module</h3>
            ...
            <form wire:submit.prevent="createModule" class="space-y-4 text-xs font-semibold">
                <div>
                    ...
                    <input type="text" wire:model="newModuleTitle" placeholder="e.g. Module 2: Advanced Eloquent & Database Relationships" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:ring-2 focus:ring-[#f15153]">
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                    ...
                    <button type="submit" class="px-5 py-2 rounded-xl bg-[#251237] text-white font-black shadow-md">Save Module</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="modalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="w-full max-w-xl rounded-3xl bg-white p-6 sm:p-8 shadow-2xl space-y-6" @click.away="modalOpen = false">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                <h3 class="text-lg font-extrabold text-[#251237]">
            ...
            <form wire:submit.prevent="saveLesson" class="space-y-4">
                ...
                <div>
                    ...
                    <select wire:model="module_id" class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-[#f15153]">
                ...
                <div>
                    ...
                    <input type="text" wire:model="title" placeholder="e.g. Introduction to MVC" class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-[#f15153]">
                ...
                <div class="p-4 rounded-2xl bg-slate-50 border border-dashed border-slate-300 space-y-2">
                    <label class="block text-xs font-bold text-[#251237] uppercase">Option 1: Upload Video / Media File (.mp4, .mp3, .webm, .mov)</label>
                    <input type="file" wire:model="videoFile" class="text-xs text-slate-600 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-[#251237] file:text-white hover:file:bg-slate-800">
                ...
                <div>
                    ...
                    <input type="text" wire:model="video_url" placeholder="storage/videos/sample.mp4 OR https://www.youtube.com/watch?v=..." class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-mono focus:ring-2 focus:ring-[#f15153]">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" wire:model="duration" class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold focus:ring-2 focus:ring-[#f15153]">
                    </div>

                    <!-- Free Preview Checkbox -->
                    <div class="flex items-center pt-6">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" wire:model="is_free_preview" class="rounded border-slate-300 text-[#f15153] focus:ring-[#f15153] w-4 h-4">
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                    ...
                    <button type="submit" wire:loading.attr="disabled" wire:target="videoFile" class="px-5 py-2.5 rounded-xl bg-[#f15153] hover:bg-red-700 disabled:opacity-50 text-white text-xs font-extrabold shadow-md">
