<div class="space-y-6" x-data="{ modalOpen: @entangle('showModal') }">
    ...
    <div style="background-color: #251237; border: 1px solid rgba(241,81,83,0.25);" class="rounded-2xl p-5 shadow-xl flex flex-col xl:flex-row xl:items-center justify-between gap-4 text-white">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <button wire:click="syncAdzunaJobs" wire:loading.attr="disabled" style="background-color: #1e0d2d; border: 1px solid rgba(241,81,83,0.3);" class="px-4 py-2.5 rounded-xl text-white text-xs font-bold shadow-md hover:bg-white/10 transition-all flex items-center gap-2 disabled:opacity-50 relative">
        ...
        <div class="flex flex-wrap items-center gap-3">
            <div class="relative w-full sm:w-[300px]">
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search jobs, companies..." style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3);" class="w-full pl-9 pr-4 py-2 rounded-xl text-xs font-medium text-white placeholder-slate-400 focus:outline-none focus:border-[#f15153]">
                <svg class="w-4 h-4 text-slate-400 absolute left-3 top-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            </div>

            <select wire:model.live="selectedCompany" style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="px-3 py-2 rounded-xl text-xs font-bold focus:outline-none">
    ...
    <div x-show="modalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div @click.away="modalOpen = false" style="background-color: #251237; border: 1px solid rgba(241,81,83,0.25);" class="rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden text-white">
            ...
            <form wire:submit.prevent="saveJob" class="p-6 space-y-4 text-xs font-semibold">
                <div>
                    ...
                    <input type="text" wire:model="title" placeholder="e.g. Senior Laravel Developer" style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3);" class="w-full px-4 py-2.5 rounded-xl text-white placeholder-slate-500 focus:outline-none">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select wire:model="company_id" style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full px-4 py-2.5 rounded-xl focus:outline-none">
                            @foreach ($companies as $comp)
                                <option value="{{ $comp->id }}" class="text-slate-900">{{ $comp->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" wire:model="salary_min" style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3);" class="w-full px-4 py-2.5 rounded-xl text-white focus:outline-none">
                    </div>

                    <div>
                        <label class="block text-slate-300 font-bold uppercase mb-1">Salary Max (Annual ₹)</label>
                        <input type="number" wire:model="salary_max" style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3);" class="w-full px-4 py-2.5 rounded-xl text-white focus:outline-none">
                    </div>
                </div>

                <div>
                    <label class="block text-slate-300 font-bold uppercase mb-1">Posting Status</label>
                    <select wire:model="status" style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3); color: white;" class="w-full px-4 py-2.5 rounded-xl focus:outline-none">
                ...
                <div>
                    ...
                    <textarea wire:model="description" rows="4" placeholder="Enter role requirements..." style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.3);" class="w-full px-4 py-2.5 rounded-xl text-white placeholder-slate-500 focus:outline-none"></textarea>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
                    ...
                    <button type="submit" style="background-color: #f15153;" class="px-6 py-2.5 rounded-xl text-white font-black shadow-md">
