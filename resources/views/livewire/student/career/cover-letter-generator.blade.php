<div class="space-y-8 text-white">
    ...
    <div style="background-color: #0B1F3A; border: 1px solid #1e3a5f;" class="rounded-3xl p-6 sm:p-8 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-300 mt-1">Generate personalized, high-converting cover letters tailored to target companies and roles.</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div style="background-color: #0B1F3A; border: 1px solid #1e3a5f;" class="rounded-3xl p-6 shadow-xl text-white space-y-5">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-[11px] font-bold text-slate-300 mb-1">Target Job Title</label>
                            <input type="text" wire:model.live.debounce.300ms="targetRole" placeholder="e.g. Software Developer Intern" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-3.5 py-2 rounded-xl text-xs font-semibold focus:outline-none focus:border-rose-500">
                        </div>
                        <div>
                            <label class="block text-[11px] font-bold text-slate-300 mb-1">Target Company Name</label>
                            <input type="text" wire:model.live.debounce.300ms="companyName" placeholder="e.g. Google, TCS, Infosys" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-3.5 py-2 rounded-xl text-xs font-semibold focus:outline-none focus:border-rose-500">
                        </div>
                    </div>

                    <div>
                        <label class="block text-[11px] font-bold text-slate-300 mb-1">Hiring Manager / Department (Optional)</label>
                        <input type="text" wire:model.live.debounce.300ms="hiringManager" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-3.5 py-2 rounded-xl text-xs font-semibold focus:outline-none focus:border-rose-500">
                    </div>

                    <div>
                        <label class="block text-[11px] font-bold text-slate-300 mb-1">Applicant Name & Location</label>
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <input type="text" wire:model.live.debounce.300ms="location" placeholder="City, State" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-3 py-2 rounded-xl text-xs font-semibold focus:outline-none focus:border-rose-500">
                        </div>
                    </div>

                    <div>
                        <label class="block text-[11px] font-bold text-slate-300 mb-1">Key Technical Stack & Skills</label>
                        <input type="text" wire:model.live.debounce.300ms="skillsInput" placeholder="e.g. Python, React, Node.js, PostgreSQL" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-3.5 py-2 rounded-xl text-xs font-semibold focus:outline-none focus:border-rose-500">
                    </div>

                    <div>
                        <label class="block text-[11px] font-bold text-slate-300 mb-1">Core Projects & Achievements to Highlight</label>
                        <textarea wire:model.live.debounce.300ms="coreHighlights" rows="3" placeholder="Describe your key projects and achievements..." style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-3.5 py-2 rounded-xl text-xs font-semibold focus:outline-none focus:border-rose-500"></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select wire:model.live.debounce.300ms="toneStyle" style="background: #112240; border: 1px solid #1e3a5f; color: white;" class="w-full px-3.5 py-2 rounded-xl text-xs font-semibold focus:outline-none focus:border-rose-500">
                            ...
                            <option value="Startup tech & innovative">Startup tech & innovative</option>
                    ...
                    <button wire:click="generateCoverLetter" wire:loading.attr="disabled" style="background-color: #D62828;" class="w-full py-3.5 rounded-xl text-white font-black text-xs shadow-lg hover:bg-rose-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                        ...
                        <span wire:loading wire:target="generateCoverLetter" class="flex items-center gap-2">
                            ...
                            <span>Generating with NVIDIA AI...</span>
                        </span>
                    </button>
                </div>
            </div>
        ...
        <div class="lg:col-span-7 space-y-6">
            ...
            <div style="background-color: #0B1F3A; border: 1px solid #1e3a5f;" class="rounded-3xl p-6 text-white shadow-xl flex items-center justify-between gap-6">
                <div>
                    ...
                    <h3 class="text-lg font-black text-white mt-0.5">
                        ...
                        <span class="text-slate-400 font-normal">at {{ $companyName ?: 'Company' }}</span>
                    </h3>
                </div>
                @if (!empty($coverLetterOutput))
            ...
            @if ($coverLetterOutput)
                <div style="background-color: #ffffff !important; color: #111827 !important;" class="rounded-xl p-10 shadow-2xl space-y-6 font-serif text-[13.5px] leading-relaxed border border-slate-300">
                    <!-- HEADER -->
                    ...
                    <div style="color: #1f2937 !important;" class="text-xs leading-snug">
                        ...
                        <div style="color: #000000 !important;" class="font-bold">{{ $companyName ?: 'Target Company' }}</div>
                    </div>

                    <!-- BODY -->
                    <div style="color: #111827 !important;" class="text-xs text-left whitespace-pre-line leading-relaxed">{{ $coverLetterOutput }}</div>
                    ...
                    @if (count($keyHighlights) > 0)
                        <div class="pt-2">
                            ...
                            <ul style="color: #1f2937 !important;" class="list-disc list-inside text-xs space-y-1 pl-2">
                    ...
                    <!-- SIGNATURE -->
                    <div style="color: #000000 !important;" class="pt-4 text-xs">
                        <div>Sincerely,</div>
                        <div class="font-bold mt-4">{{ $fullName ?: 'Your Name' }}</div>
                    </div>
                </div>
            @else
                <!-- EMPTY STATE -->
                <div style="background-color: #0B1F3A; border: 1px solid #1e3a5f;" class="rounded-3xl p-10 text-center shadow-xl">
                    ...
                    <p class="text-xs text-slate-400 max-w-sm mx-auto">Fill in the target role and company details, then click <strong class="text-rose-400">Generate</strong> to create a personalized AI cover letter.</p>
                </div>
            @endif
        </div>
    </div>
