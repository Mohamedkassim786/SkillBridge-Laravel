<div class="min-h-screen pb-20" style="background-color: #321847; color: #d4c5e2;">
    ...
    <div style="background: linear-gradient(180deg, #321847 0%, #210f30 100%); border-bottom: 1px solid rgba(241,81,83,0.25); padding: 24px 0 28px;">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="flex items-center gap-2 text-xs font-semibold text-purple-300 mb-3">
                ...
                <span class="text-[#f15153] font-bold">Courses</span>
            </nav>

            <h1 class="text-4xl font-black text-white tracking-tight mb-2">Explore Our Courses</h1>
            <p class="text-sm max-w-2xl" style="color: #d4c5e2;">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6">
        ...
        <div style="background: #251237; border: 1px solid rgba(241,81,83,0.2); border-radius: 16px; padding: 16px 24px;" class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            
            <!-- Search Bar (Left) -->
            <div class="relative flex-1 max-w-sm" style="display: flex; align-items: center;">
                <svg style="width: 16px; height: 16px; color: #a997be; position: absolute; left: 14px; pointer-events: none;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search courses..." style="background: #1e0d2d; border: 1px solid rgba(255,255,255,0.12); color: white; padding-left: 42px; padding-right: 16px; padding-top: 10px; padding-bottom: 10px; font-size: 13px; border-radius: 12px; width: 100%; outline: none;">
            </div>

            <!-- Results Count & Sort (Right) -->
            <div class="flex flex-wrap items-center gap-6 text-xs shrink-0">
                <p class="font-medium" style="color: #d4c5e2;">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @forelse ($courses as $index => $course)
                ...
                <div style="background: #251237; border: 1px solid rgba(241,81,83,0.2); border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.25); overflow: hidden; display: flex; flex-direction: column; justify-content: space-between; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-4px)'; this.style.boxShadow='0 12px 30px rgba(0,0,0,0.4)';" onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 8px 24px rgba(0,0,0,0.25)';">

                    <div>
                        <!-- Image Container & Badges -->
                        <div style="position: relative; height: 165px; overflow: hidden; background: #1e0d2d;">
                            <img src="{{ $bannerImg }}" alt="{{ $course->title }}" style="width: 100%; height: 100%; object-fit: cover;">

                            <!-- 40% OFF Badge (Top Left) -->
                            <span style="position: absolute; top: 12px; left: 12px; background: #f15153; color: white; font-weight: 800; font-size: 10px; padding: 4px 10px; border-radius: 20px; box-shadow: 0 2px 8px rgba(241,81,83,0.4);">
                                40% OFF
                            </span>

                            <!-- Category Badge (Top Right) -->
                            <span style="position: absolute; top: 12px; right: 12px; background: rgba(33, 15, 48, 0.9); backdrop-filter: blur(4px); color: #f15153; font-weight: 700; font-size: 10.5px; padding: 4px 10px; border-radius: 20px; border: 1px solid rgba(241,81,83,0.3);">
                        ...
                        <div style="padding: 16px 16px 10px; display: flex; flex-direction: column; gap: 10px;">
                            ...
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <div style="width: 24px; height: 24px; border-radius: 50%; background: #f15153; display: flex; align-items: center; justify-content: center; color: white; font-weight: 800; font-size: 10px; flex-shrink: 0;">
                                    {{ strtoupper(substr($instructor, 0, 1)) }}
                                </div>
                                <span style="font-size: 12px; font-weight: 600; color: #a997be;">by {{ $instructor }}</span>
                            </div>
                            ...
                            <!-- Rating & Enrolled -->
                            <div style="display: flex; align-items: center; justify-content: space-between; font-size: 11.5px; color: #d4c5e2;">
                                <div style="display: flex; align-items: center; gap: 4px;">
                                    <span style="color: #f59e0b; font-weight: 800;">5.0 ★★★★★</span>
                                    <span style="color: #a997be;">(Verified)</span>
                                </div>
                                <span style="color: #a997be; font-weight: 600;">{{ $enrolledCount }} {{ Str::plural('student', $enrolledCount) }}</span>
                            </div>

                            <!-- Meta Info: Duration | Lectures | Level -->
                            <div style="font-size: 11px; color: #a997be; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.08); display: flex; align-items: center; justify-content: space-between;">
                                <span class="inline-flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5 text-purple-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                ...
                                <span style="color: #f15153; font-weight: 700;">{{ ucfirst($course->currentVersion?->level ?? 'Beginner') }}</span>
                            </div>

                            <!-- Price Section -->
                            <div style="display: flex; align-items: baseline; gap: 8px; margin-top: 2px;">
                                <span style="font-size: 20px; font-weight: 900; color: #f15153;">₹{{ number_format($priceVal) }}</span>
                            </div>
                    ...
                    <div style="padding: 0 16px 16px;">
                        <a href="{{ route('courses.show', $course->id) }}" style="background: #f15153; color: white; font-weight: 800; font-size: 13px; padding: 10px; border-radius: 10px; text-align: center; text-decoration: none; display: block; box-shadow: 0 4px 14px rgba(241,81,83,0.35); transition: opacity 0.15s;" class="hover:opacity-90">
                            Enroll Now ➔
                        </a>
                    </div>
            ...
            @empty
                <div style="background: #251237; border: 1px solid rgba(241,81,83,0.2); border-radius: 16px; padding: 48px;" class="col-span-full text-center" style="color: #a997be;">
        ...
        <div class="pt-6 flex justify-center">
            <div style="background: #251237; border: 1px solid rgba(241,81,83,0.2); border-radius: 14px; padding: 8px 16px;" class="flex items-center gap-2 text-xs font-bold text-white">
                ...
                <span class="px-3 py-1.5 rounded-lg hover:bg-white/10 cursor-pointer">12</span>
        ...
        <div style="background: linear-gradient(135deg, #251237 0%, #210f30 100%); border: 1px solid rgba(241,81,83,0.25); border-radius: 24px; padding: 40px 32px; margin-top: 64px;" class="text-center max-w-4xl mx-auto space-y-4">
            ...
            <form onsubmit="event.preventDefault(); alert('Subscribed successfully!');" class="flex flex-col sm:flex-row items-center justify-center gap-3 max-w-md mx-auto pt-2">
                <input type="email" placeholder="Enter your email..." required style="background: #1e0d2d; border: 1px solid rgba(255,255,255,0.12); color: white;" class="w-full px-4 py-3 rounded-xl text-xs placeholder-purple-300 focus:outline-none focus:border-[#f15153]">
