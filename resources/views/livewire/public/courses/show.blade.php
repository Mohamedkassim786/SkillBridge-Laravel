    ...
    <div style="background: linear-gradient(180deg, #321847 0%, #210f30 100%); border-bottom: 1px solid rgba(241,81,83,0.25); padding: 24px 0 28px;">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="flex items-center gap-2 text-xs font-semibold text-purple-300 mb-3">
                ...
                <span class="text-purple-400">></span>
                <span class="text-[#f15153] font-bold max-w-xs truncate">{{ $course->title }}</span>
            </nav>
            ...
            <div class="max-w-4xl space-y-4">
                ...
                <div class="flex flex-wrap items-center gap-6 pt-2 text-xs">
                    <div class="flex items-center gap-3">
                        <div style="width: 48px; height: 48px; border-radius: 50%; background: #f15153; display: flex; align-items: center; justify-content: center; color: white; font-weight: 900; font-size: 18px; box-shadow: 0 4px 14px rgba(241,81,83,0.35);" class="shrink-0">
                        ...
                        <div>
                            ...
                            <div style="font-size: 11.5px; color: #a997be;">Senior Laravel Developer @ TCS</div>
                        </div>
                    </div>

                    <div class="h-8 w-px bg-purple-800/40 hidden sm:block"></div>

                    <div class="flex items-center gap-2">
                        <span class="text-amber-400 font-bold text-sm">4.8 ★★★★★</span>
                        <span style="color: #a997be;">(1,234 reviews)</span>
                        <span style="color: #a997be;">•</span>
                        <span class="text-white font-bold">15,432 students</span>
                    </div>
                </div>

                <!-- Course Meta Info Bar -->
                <div class="flex flex-wrap items-center gap-5 pt-4 text-xs text-purple-200 border-t border-purple-800/40">
                    <span class="flex items-center gap-1.5"><svg class="w-4 h-4 text-[#f15153]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> 42 hours total</span>
                    <span class="flex items-center gap-1.5"><svg class="w-4 h-4 text-purple-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg> 156 video lectures</span>
                    ...
                    <span class="flex items-center gap-1.5"><svg class="w-4 h-4 text-[#f15153]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg> Lifetime access</span>
        ...
        <div class="course-detail-layout">
            ...
            <div class="space-y-10">
                ...
                <div style="background: #251237; border: 1px solid rgba(241,81,83,0.25); border-radius: 24px; padding: 32px;" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-purple-200">
                ...
                <div style="background: #251237; border: 1px solid rgba(241,81,83,0.25); border-radius: 24px; padding: 32px;" class="space-y-6">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-xs font-semibold" style="color: #a997be;">{{ $dbModules->count() }} Modules • {{ $totalLecturesCount }} Lectures</span>
                    </div>

                    <div class="space-y-3" x-data="{ openMod: 0 }">
                        @forelse ($dbModules as $mIdx => $mod)
                            <div style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.2); border-radius: 16px;" class="overflow-hidden">
                                <button @click="openMod = openMod === {{ $mIdx }} ? null : {{ $mIdx }}" class="w-full px-5 py-4 text-left flex items-center justify-between hover:bg-white/5 transition-colors">
                                    <div class="flex items-center gap-3">
                                        <span style="background: rgba(241,81,83,0.15); color: #f15153;" class="w-7 h-7 rounded-lg text-xs font-extrabold flex items-center justify-center border border-rose-500/30">
                                            {{ $mIdx + 1 }}
                                        </span>
                                        <span class="text-sm font-bold text-white">{{ $mod->title }}</span>
                                    </div>
                                    <div class="flex items-center gap-4">
                                        <span class="text-xs hidden sm:inline" style="color: #a997be;">{{ $mod->lessons->count() }} {{ Str::plural('lecture', $mod->lessons->count()) }}</span>
                                        <svg class="w-4 h-4 text-purple-300 transition-transform" :class="openMod === {{ $mIdx }} ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                                    </div>
                                </button>
                                <div x-show="openMod === {{ $mIdx }}" x-cloak class="px-5 py-3 border-t border-purple-800/40 space-y-2 text-xs text-purple-100">
                                    @forelse ($mod->lessons as $lesIdx => $les)
                                        <div class="flex items-center justify-between py-1 border-b border-purple-800/30 last:border-0">
                                            <span class="flex items-center gap-2">
                                                <svg class="w-4 h-4 {{ $les->is_free_preview ? 'text-[#f15153]' : 'text-purple-300' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                                Lecture {{ $lesIdx + 1 }}: {{ $les->title }}
                                            </span>
                                            @if ($les->is_free_preview)
                                                <span class="text-emerald-400 font-bold text-[10.5px]">Free Preview</span>
                                            @else
                                                <span style="color: #a997be;">{{ (int) round($les->duration / 60) }} min</span>
                                            @endif
                                        </div>
                                    @empty
                                        <div class="py-1 italic" style="color: #a997be;">No lessons added to this module yet.</div>
                                    @endforelse
                                </div>
                            </div>
                        @empty
                            <div class="p-6 rounded-2xl border text-center text-xs" style="background: #1e0d2d; border-color: rgba(255,255,255,0.1); color: #a997be;">
                ...
                <div style="background: #251237; border: 1px solid rgba(241,81,83,0.25); border-radius: 24px; padding: 32px;" class="space-y-4">
                    ...
                    <ul class="space-y-2.5 text-xs list-disc list-inside" style="color: #d4c5e2;">
                ...
                <div style="background: #251237; border: 1px solid rgba(241,81,83,0.25); border-radius: 24px; padding: 32px;" class="space-y-6">
                    ...
                    <div class="flex flex-col sm:flex-row items-start gap-6">
                        <div style="width: 100px; height: 100px; border-radius: 50%; background: #f15153; display: flex; align-items: center; justify-content: center; color: white; font-weight: 900; font-size: 36px; box-shadow: 0 6px 20px rgba(241,81,83,0.4);" class="shrink-0">
                            {{ strtoupper(substr($course->trainer?->name ?? 'John Doe', 0, 1)) }}
                        </div>
                        <div class="space-y-3 flex-1">
                            <div>
                                <h3 class="text-xl font-extrabold text-white">{{ $course->trainer?->name ?? 'John Doe' }}</h3>
                                <div class="text-xs font-semibold" style="color: #f15153;">Senior Laravel Developer @ TCS</div>
                            </div>
                            ...
                            <div class="flex items-center gap-4 pt-1">
                                <a href="#" style="color: #f15153;" class="text-xs font-bold hover:underline">LinkedIn</a>
                                <a href="#" style="color: #d4c5e2;" class="text-xs font-bold hover:underline">GitHub</a>
                                <a href="#" style="color: #c084fc;" class="text-xs font-bold hover:underline">Twitter</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div style="background: #251237; border: 1px solid rgba(241,81,83,0.25); border-radius: 24px; padding: 32px;" class="space-y-6">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-8 p-6 rounded-2xl" style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.2);">
                        <div class="text-center sm:text-left space-y-1">
                            <div class="text-5xl font-black text-white">4.8</div>
                            <div class="text-amber-400 text-lg">★★★★★</div>
                            <div class="text-xs" style="color: #a997be;">Course Rating • 1,234 Reviews</div>
                        </div>

                        <div class="flex-1 space-y-2 text-xs w-full">
                            <div class="flex items-center gap-3">
                                <span class="w-8" style="color: #a997be;">5 ★</span>
                                <div class="flex-1 h-2 rounded-full bg-purple-950 overflow-hidden"><div class="h-full bg-amber-400 rounded-full" style="width: 85%;"></div></div>
                                <span class="w-8 text-right" style="color: #a997be;">85%</span>
                            </div>
                            <div class="flex items-center gap-3">
                                <span class="w-8" style="color: #a997be;">4 ★</span>
                                <div class="flex-1 h-2 rounded-full bg-purple-950 overflow-hidden"><div class="h-full bg-amber-400 rounded-full" style="width: 12%;"></div></div>
                                <span class="w-8 text-right" style="color: #a997be;">12%</span>
                            </div>
                            <div class="flex items-center gap-3">
                                <span class="w-8" style="color: #a997be;">3 ★</span>
                                <div class="flex-1 h-2 rounded-full bg-purple-950 overflow-hidden"><div class="h-full bg-amber-400 rounded-full" style="width: 2%;"></div></div>
                                <span class="w-8 text-right" style="color: #a997be;">2%</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        @foreach ($reviewsSample as $rev)
                            <div style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.2); border-radius: 16px; padding: 20px;" class="space-y-3">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        <div style="width: 32px; height: 32px; border-radius: 50%; background: #f15153; color: white; font-weight: 800; font-size: 12px; display: flex; align-items: center; justify-content: center;">
                                            {{ strtoupper(substr($rev['name'], 0, 1)) }}
                                        </div>
                                        <div>
                                            <div class="text-sm font-bold text-white">{{ $rev['name'] }}</div>
                                            <div class="text-[11px] text-amber-400">★★★★★ <span class="ml-1" style="color: #a997be;">{{ $rev['date'] }}</span></div>
                                        </div>
                                    </div>
                                ...
                                <p class="text-xs leading-relaxed italic" style="color: #d4c5e2;">
                                    "{{ $rev['quote'] }}"
                                </p>
                            </div>
                        @endforeach
                    ...
                    <div class="text-center pt-2">
                        <button class="text-xs font-bold text-[#f15153] hover:underline">View all 1,234 reviews ➔</button>
                    </div>
                </div>
            ...
            <div style="position: sticky; top: 90px; z-index: 10;">
                <div style="background: #251237; border: 1px solid rgba(241,81,83,0.25); border-radius: 24px; padding: 24px; box-shadow: 0 12px 36px rgba(0,0,0,0.35);" class="space-y-6">

                    <!-- Course Preview Video Container -->
                    <div style="position: relative; height: 180px; border-radius: 16px; overflow: hidden; background: #1e0d2d;">
                        ...
                        <div style="position: absolute; inset: 0; background: rgba(50,24,71,0.5); display: flex; align-items: center; justify-content: center;">
                            <div style="width: 56px; height: 56px; border-radius: 50%; background: #f15153; color: white; display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 20px rgba(241,81,83,0.5); cursor: pointer;" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                    ...
                    <div class="space-y-1">
                        <div class="flex items-center justify-between">
                            <span style="background: #f15153; color: white;" class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider">
                                40% OFF - Limited Time!
                            </span>
                        </div>
                        <div class="flex items-baseline gap-3 pt-2">
                            <span class="text-3xl font-black text-[#f15153]">₹{{ number_format($course->currentVersion?->price ?? 2999) }}</span>
                            <span class="text-sm line-clamp-1 line-through" style="color: #a997be;">₹{{ number_format(($course->currentVersion?->price ?? 2999) * 1.66) }}</span>
                        </div>
                    </div>

                    <!-- Action Buttons -->
                    <div class="space-y-3">
                        <button wire:click="enroll" style="background: #f15153; color: white; box-shadow: 0 4px 16px rgba(241,81,83,0.4);" class="w-full py-3.5 rounded-xl font-extrabold text-sm hover:opacity-90 transition-all text-center block">
                            Enroll Now ➔
                        </button>
                        <button style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); color: white;" class="w-full py-3 rounded-xl font-bold text-xs hover:bg-white/15 transition-all">
                            Add to Cart
                        </button>
                    </div>

                    <!-- Guarantee Badge -->
                    <div style="background: #1e0d2d; border: 1px solid rgba(241,81,83,0.2); border-radius: 12px; padding: 12px;" class="flex items-center gap-3 text-xs text-purple-200">
                        ...
                        <span><strong>30-Day Money-Back Guarantee</strong><br><span class="text-[11px]" style="color: #a997be;">100% Risk-Free Guarantee</span></span>
                    </div>

                    <!-- This Course Includes List -->
                    <div class="space-y-3 text-xs text-purple-200 border-t border-purple-800/40 pt-4">
                    ...
                    <!-- Buy for your team -->
                    <div class="text-center pt-2 border-t border-purple-800/40">
                        <a href="#" style="color: #f15153;" class="text-xs font-bold hover:underline">Buy for your team (Corporate Discount) ➔</a>
                    </div>
        ...
        <div class="mt-16 pt-10 border-t border-purple-800/40 space-y-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @forelse ($relatedCourses as $rel)
                    <div style="background: #251237; border: 1px solid rgba(241,81,83,0.2); border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.25); overflow: hidden; display: flex; flex-direction: column; justify-content: space-between;">
                        <div>
                            <div style="position: relative; height: 150px; background: #1e0d2d;">
                                ...
                                <span style="position: absolute; top: 10px; left: 10px; background: #f15153; color: white; font-weight: 800; font-size: 10px; padding: 3px 8px; border-radius: 20px;">
                                    40% OFF
                                </span>
                            </div>
                            <div style="padding: 14px; display: flex; flex-direction: column; gap: 8px;">
                                ...
                                <div style="font-size: 11.5px; color: #a997be;">by {{ $rel->trainer?->name ?? 'John Doe' }}</div>
                                <div style="font-size: 16px; font-weight: 900; color: #f15153;">₹{{ number_format($rel->currentVersion?->price ?? 2999) }}</div>
                            </div>
                        </div>
                        <div style="padding: 0 14px 14px;">
                            <a href="{{ route('courses.show', $rel->id) }}" style="background: #f15153; color: white; font-weight: 800; font-size: 12px; padding: 8px; border-radius: 8px; text-align: center; display: block; text-decoration: none;">
                                View Syllabus ➔
                            </a>
                        </div>
                    </div>
                @empty
                    <div class="col-span-full text-xs text-center py-4" style="color: #a997be;">No related courses found.</div>
                @endforelse
            </div>
        </div>
